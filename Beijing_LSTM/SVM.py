...
if __name__=="__main__":
    ...
    data=pd.read_csv("error_dataset.csv")
    data_X=data['value'].values
    data_Y=data['class'].values
    train_X,test_X,train_Y,test_Y=train_test_split(data_X,data_Y,test_size=0.4,random_state=42)
    train_data_X,train_data_Y,test_data_X,test_data_Y=crate_data_to_target(train_X,train_Y,test_X,test_Y)

    np.savetxt("svm_test_x.txt",test_data_X)
    np.savetxt("svm_test_y.txt",test_data_Y)
    #定义一些常量和占位符
    batch_size = 100
            ...
            for p in range(rand_y.shape[1]):
                for j in range(rand_y.shape[0]):
                    if rand_y[j][p]==1:
                        true_label.append(j)
                        continue
            true_label=np.array(list(true_label))
            pred_output=np.array(list(pred_output))
            print("true_value:",true_label)
            print("pred_value:",pred_output)
            print("f1_score:", f1_score(true_label, pred_output, average='macro'))
            print("accuracy:", accuracy_score(true_label, pred_output))
            print("precision:", precision_score(true_label, pred_output, average='macro'))
            print("Recall:", recall_score(true_label, pred_output, average='macro'))
            fpr, tpr,thresholds = roc_curve(true_label,pred_output, pos_label=1)

            #获取最好的分数
            if max_score['f1_score']<f1_score(true_label, pred_output, average='macro'):
                max_score['f1_score']=f1_score(true_label, pred_output, average='macro')
                max_score['accuracy']=accuracy_score(true_label, pred_output)
                max_score['precision']=precision_score(true_label, pred_output, average='macro')
                max_score['Recall']=recall_score(true_label, pred_output, average='macro')
                max_score['index']=i-1
            print("auc:", auc(fpr,tpr))
            print("epoch:{}=======loss:{:.9f}========accuracy:{:.9f}".format(str(i+1),temp_loss,temp_accuracy))
    print(max_score)
    save_path=saver.save(sess,"SVM_Model/SVM.ckpt")
    print("model has been stored in {}".format(save_path))
